  ...
  methods: {
    ...
    drawLines() {
      ...
      if (this.showLines) {
        ...
        // 画竖直标准线，防止越界
        const verticalLineX = canvas.width / 2 + this.offsetX;
        if (verticalLineX >= 0 && verticalLineX <= canvas.width) {
          context.beginPath();
          context.moveTo(verticalLineX, fixX);
          context.lineTo(verticalLineX, canvas.height + fixX);
          context.stroke();
        }

        // 画水平标准线，防止越界
        const horizontalLineY = canvas.height / 2 + this.offsetY;
